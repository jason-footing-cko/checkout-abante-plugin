...
class Model_Methods_creditcard extends Model_Methods_Abstract implements Model_Interface
{
  ...
  public function getData() {
    ...
    $toReturn = array(
        ...
        'logourl' => $this->config->get('checkoutapipayment_logourl'),
        'buttoncolor' => $this->config->get('checkoutapipayment_buttoncolor'),
        'iconcolor' => $this->config->get('checkoutapipayment_iconcolor'),
        'themecolor' => $this->config->get('checkoutapipayment_themecolor'),
        'currencyformat' => ($this->config->get('checkoutapipayment_currencycode') == 'code') ? 'true' : 'false',
  ...
  protected function _placeorder($order_info) {
    $config = array();
    
    if (!empty($this->request->post['cko-cc-redirectUrl'])) {
      
      $this->load->model('checkout/order');
      $message = 'Your order has been created successfully' ;
      $this->model_checkout_order->confirm($this->session->data['order_id'], ORDER_PENDING, $message);
      $json['success'] = html_entity_decode($this->request->post['cko-cc-redirectUrl'] .'&trackId=' . $this->session->data['order_id']);
      $this->load->library('json');
      $this->response->setOutput(AJson::encode($json));
      
    }
    else {  
      parent::_placeorder($order_info);
